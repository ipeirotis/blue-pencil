---
name: blue-pencil
description: Revise, copy-edit, line-edit, polish, tighten, or give editorial feedback on existing academic prose; make a paper clearer to non-specialists, less AI-sounding, and more human to read; read a whole paper cold; check cross-section consistency; cut toward a length limit; respond to reviewer comments; or draft and improve a response-to-reviewers letter. Diagnoses logical flow, argumentation, exposition, narrative spine, copyediting, and reader experience while preserving the author's voice, citations, technical meaning, and numerical claims. Not for drafting new sections from notes, checking data or analyses, verifying citations or novelty, citation formatting or BibTeX, LaTeX compilation, pure typo lists, or non-academic prose.
license: MIT
...
- Wants numbers recomputed, a data analysis run, or a figure regenerated from
  data. Blue Pencil treats numbers and figures as protected content; it does
  not inspect data, execute analysis code, or re-render figures.
- Wants citations checked against their sources or a contribution scanned for
  prior work. Blue Pencil preserves citations; it does not retrieve literature
  or assess novelty.
...
skill's tool surface performs no computation or retrieval, so the only verified
substance is the author's own. Substance means manuscript content, stated in or
about the paper: editorial reporting about your own edit (the approximate `Word count:` line,
...
Use old information first, new information last. Prefer concrete subjects and active verbs. Keep the core of a clause together, every element next to the word that completes it (a subject with its verb, a linking verb with its complement, a verb with its object or particle, an object with its complement): an interrupting constituent that opens in any such gap (an appositive, a parenthetical, a non-restrictive relative clause, an interposed adverbial), or a chain of modifiers that holds the completing word out of reach, forces the reader to hold an unfinished clause, so close the gap, moving it to a trailing clause or its own sentence or bringing the completing word up (`references/sentence-patterns.md`, the interrupted clause, and its sibling the left-branching preamble). Use technical terms consistently. Cut throat-clearing.
...
Subtraction is the highest-yield edit and the easiest to botch. Two operations carry different risk: *compress* (fewer words, same content) is near zero-risk, so apply it freely; *delete* (remove a whole unit) changes what the text says, so apply the keep-test first: if this goes, what does the reader lose? A unit earns its place if it advances the thesis, makes a claim believable, links two ideas the reader would not otherwise connect, serves a reader the other sentences do not, pre-empts a predictable objection, or sets rhythm. If it does none, cut it; if it does one, compress but keep. Most drafts carry 15 to 25% that does not serve the story: treat that figure as a prior, not a quota, and never manufacture cuts from a draft that is already tight.
...
Load `references/subtraction.md` for the full keep-test with its catalog of destructive cuts, the say-it-once rule, the scope-versus-filler reading-out test, the blind spot (subtraction never finds the missing step), and a worked example.
...
Three directives, whose exceptions and worked pairs live in `references/altitude.md`. Report the finding, not the machinery: above Results, a result is direction plus magnitude plus one significance marker, each clause of the apparatus sent down to the section the manuscript actually uses for it; simplify the wording, never the estimand (a cut in time is not a gain in speed), and keep its unit and estimate status. Send each caveat to its home section: a design qualification to Methods, a sample-size limitation to Limitations, never voiced at the moment the claim it qualifies is first stated; but a power qualifier on a null headline finding ("no detectable difference at this sample size") is scope that stays with the claim. Keep one number per claim: a confidence interval or secondary p-value stacked on the same headline figure rides down into the body, while an intrinsic keeper stays (a venue-required interval, a range whose spread is itself the finding, a load-bearing derived metric, or a baseline that makes a relative effect interpretable: "mortality fell 50%, from 2% to 1%").

The fix for a too-high clause is relocation, not compression or deletion, and the gates in `references/altitude.md` bind every move: relocate only when the home section is inside the unit you were asked to edit and the stage is first draft or a whole-section pass; on a response-to-reviewers pass, only when both the source and destination paragraphs are flagged; otherwise flag the altitude issue or route the move to `Author questions`, never cutting the clause instead. Moving a numerical clause still trips the numerical-claim constraint, so flag it too. Two nearby rules finish the job: say it once (a restatement that adds no claim and serves no reader is a cut, per the Subtraction section), and the load-bearing sentence is the plainest (the sentence carrying the main point should be the easiest to parse, not the most nested).
...
Good (introduction, when Section 6 preserves the claim): "The method cuts error rates in the settings we study; Section 6 maps where the effect is strongest." The dataset-scope qualifier stays, the hedge stack goes, and the caveat wall becomes the one signpost.
...
- No interrupting constituent (appositive, parenthetical, non-restrictive relative clause, interposed adverbial) and no chain of modifiers holds a clause's core open, separating any element from the word that completes it (a subject from its verb, a linking verb from its complement, a verb from its object or particle, or an object from its complement), and no stack of subordinate clauses precedes the core; each clause, embedded ones included, parses in one pass, though a brief parenthetical the reader resolves without losing the core may stay.
...
Before producing the rewrite, identify three to five voice tics from the original and preserve them. A voice tic is a stable, deliberate choice across pronoun policy, sentence length, connective vocabulary, citation placement, punctuation, or lexical preferences. Nominalisations, throat-clearing, em-dashes, banned transitions, hedge stacks, and interrupting constituents that split the core of a clause (appositives, parentheticals, non-restrictive relative clauses, interposed adverbials), separating any element from the word that completes it (a subject from its verb, a linking verb from its complement, a verb from its object or particle, or an object from its complement), are not voice tics; the style rules in `references/ai-tells-to-avoid.md` win over any voice tic unless an explicit `style_overrides:` line in `<paper_context>` sets them aside (see the Constraints section; protection rules never yield).
