...
Good (abstract): "users took about 31% less time per query (p = 0.03)." Simplify the wording, not the estimand: a cut in time is not a gain in speed, and keep the unit ("per query"), so the quantity stays what the source measured.
...
- No relocatable detail sits above its home section: no inferential machinery, stacked interval, secondary statistic, or tertiary derived rate that belongs in Results or Methods. A caveat, a venue-required or precision-carrying interval, a fragility-revealing robustness sweep, or a load-bearing derived metric with no in-scope home is not a violation, since it is kept rather than cut.
...
- Altitude check: in an abstract, introduction, or section opener, any inferential machinery, stacked interval, secondary statistic, or tertiary derived rate was relocated to its home section, or (when that home is out of scope, or the stage is final polish or a non-flagged response-to-reviewers paragraph) flagged rather than moved or cut. A caveat, a venue-required or precision-carrying interval, a fragility-revealing robustness sweep, or a load-bearing derived metric was kept or routed to `Author questions`, never silently dropped.
