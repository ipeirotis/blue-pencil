...
| `/paper:verify-citations` | Check that cited sources support their claims and scan novelty; needs literature retrieval. |
| `/paper:loop <paper>` | Plan and drive a whole-paper edit, section by section, pausing at each author checkpoint. |

The analyst commands (`verify-numbers`, `figures`, `analyze`) and `verify-citations` only propose: they never edit your manuscript, code, or data, and they say so when the tools they need are missing.
