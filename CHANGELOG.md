...
- `references/subtraction.md` gains two subsections. "Say it once" makes a restated finding a cut and distinguishes restatement (the same claim twice) from a genuine next step (an inference the first sentence did not state). "Universal qualifiers are filler, not scope" reconciles against the existing qualifier-is-content rule: a scope qualifier ("on the held-out set") rules out a reading and is content, a universal qualifier ("at this sample size", "observable") rules out nothing and cuts freely, with the reading-out test to separate them and the tie broken toward keeping.
