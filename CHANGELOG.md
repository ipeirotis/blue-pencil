...
- `install.sh`: `--uninstall` now removes only the files this installer ships (enumerated from the clone) and an unmodified agent, preserving a user's own files in the `paper/` namespace (including an older manual copy) and a customized `paper-reviser.md`. The managed `paper/` directory is removed only when nothing unmanaged remains.
