...
- `SKILL.md` gains "The revision sweep: walk the references in order", a required, ordered schedule of the eleven per-section reference-passes (logic and structural checks, section lens, exposition, altitude, precision budget, narrative spine, reader experience, line and sentence craft, subtraction, style and AI tells, copyediting) with the gate that decides when each runs. It states that the sweep is the consolidated, binding form of the file's scattered "load `references/...`" triggers, that a clean result is recorded as "no change" rather than dropped as a skipped pass, and that `cold-read.md`, `consistency-checks.md`, `analysis-integrity.md`, and `literature-checks.md` sit outside the per-section sweep because they drive their own whole-paper passes or belong to the analyst and scholar lanes. The section also states the conflict-resolution lattice that keeps the ordered passes from fighting over the same words: hard constraints and protected content above every pass, an earlier pass outranking a later one on a direct clash, the Restraint rule stopping a later pass from churning an earlier pass's result, the designed hand-offs that reconcile the recurring collisions (the keep-test protecting the rhythm sentences reader experience adds, the style scrub winning over voice tics but never over a constraint, an epistemic keeper surviving precision budget's deferral), and an `Author question` for any tension the order cannot settle.
- The Change rationale output contract gains a mandatory `References loaded:` line naming the references the sweep loaded, and `scripts/check-examples.sh` now asserts every example carries it. All seven `examples/*.md` gain the line, each reflecting its section type and stage (a first-draft introduction runs the full sweep; a methods or results paragraph gates precision-budget off; final polish runs the sentence-level sweep only; a response-to-reviewers pass applies the sweep inside the flagged paragraphs).
...
- `references/ai-tells-to-avoid.md` gains two entries and two checklist items. The concessive see-saw (every claim counterweighted the instant it is made, so the passage nets to zero) joins "Template sentence shapes", distinguished from a real limitation by the qualifier-is-content rule; the self-answering rhetorical question used as a transition joins "Other LLM tells to watch for", distinguished from the existing empty-meta-commentary tell.
- `references/sentence-patterns.md` gains three sections. "Paragraph-shape variety" is added as a cohesion move beside "Sentence-length variety", covering the claim-then-three-supports-then-restatement mold that repeats across a section. "The emphatic cleft habit" ("It is X that Y", "What Y does is X") follows the expletive-constructions section, with the fix keyed to whether the cleft's contrastive focus is real, since unlike a dummy "it is" it cannot simply be deleted. "Accumulation instead of argument" follows "Misused connectives", covering coordinate claims strung by additive connectives where one should depend on another, with the naming-a-relation fix gated on the author's own material (an unsupported relation is an `Author questions` item, not an edit).
- `references/reader-pleasure.md`'s "List rhythm" anti-pattern now points at the two named forms of it in `references/sentence-patterns.md` (accumulation and paragraph-shape uniformity), and `SKILL.md`'s read-cold self-check adds uniform paragraph shape beside uniform sentence length as a tell.
- `VERSION`, `SKILL.md` `metadata.version`, and the `README.md` badge now report 2.3.0.

## [2.2.0] - 2026-07-26

Widens the interrupted-clause check and adds its machine-generated forms, drawn from author feedback that the 1.36.0 rule under-triggered. The rule named "an appositive or parenthetical", but two common LLM interrupters are neither: a non-restrictive relative clause set into the subject-verb gap ("The interface, which relocates rather than removes the visible work of querying, produced no reduction in task time") and an interposed adverbial, often stacked with a connective ("The results, however, when compared against the baseline, suggest a smaller effect"). A literal reading of the checklist item let both slip through. This release widens the wording to "interrupting constituent" (any syntactic unit set into a gap the clause core has not yet closed, the grammatical type mattering only for where the trailing material reattaches) in all five places the phrase was restated, adds the two widening pairs to the pattern's table, and adds three LLM-specific variants as worked pairs: the first-mention gloss (composed with the first-mention-versus-full-statement rule in `references/precision-budget.md`), the prepositional chain that holds off the verb (the mirror image of a noun pile-up, cross-linked both ways), and the split correlative (tied to the "not only ... but also" tell in `references/ai-tells-to-avoid.md`). It also adds a sibling pattern, "The left-branching preamble", for a stack of subordinate clauses that precedes the core rather than interrupting it: same working-memory cost, but the gap-closing fix does not apply, so the fix fronts the core and trails the preamble. No rule changes to the existing pairs, and every fix stays a change in syntax that carries scope and calibration qualifiers through unchanged.

### Changed

- `references/sentence-patterns.md`'s "The interrupted clause" opens on "interrupting constituent" and names the constituent types (appositive, parenthetical, non-restrictive relative clause, interposed adverbial), stating that position, not grammatical type, is the test. Its table gains a non-restrictive-relative-clause row and an interposed-adverbial row, and a new "Machine-generated variants" subsection carries the first-mention gloss, the prepositional chain, and the split correlative, each with its cross-reference. The diagnostic now checks for any interrupting constituent whatever its type and for a chain of modifiers that holds a verb out of reach. A new sibling section, "The left-branching preamble", handles the pre-core stack with a front-the-core fix and its own diagnostic.
- `references/sentence-patterns.md`'s "Noun pile-ups" diagnostic points at the prepositional-chain variant as its mirror-image failure, and `references/precision-budget.md`'s first-mention-versus-full-statement rule points at the first-mention gloss.
- `SKILL.md` widens the five restatements of the rule (the "Writing quality" principle, the restraint checklist's verbatim-return gate, the voice-tic exclusion list, and the read-cold self-check) from "appositive or parenthetical" to "interrupting constituent", naming the constituent types and adding the modifier-chain and left-branching-preamble cases to the "Writing quality" principle, the restraint gate, and the self-check.
- `VERSION`, `SKILL.md` `metadata.version`, and the `README.md` badge now report 2.2.0.
...
Completes the whole-paper loop's response-to-reviewers routing so it names the full response suite. An audit of `/paper:loop` against the skill's fifteen commands found that fourteen are invoked or routed directly, and the fifteenth, `/paper:letter`, was reachable only transitively: when the stage is `response to reviewers` the loop refuses to run and hands off to the response suite, but Step A named only `/paper:triage` and `/paper:rebut`, stopping short of the `/paper:consistency` sweep and the `/paper:letter` close that `/paper:triage` itself recommends as the order of work. The handoff therefore relied on `/paper:triage`'s output to complete the picture rather than stating it. This release makes the loop's routing state the same order `/paper:triage` does, so the two routings agree and every command in the skill is named by the loop that orchestrates them. No behavioral change to the loop's own passes; this is a completeness fix to the reviewer-response handoff, and it introduces no new stage behavior (the loop still refuses to run at `response to reviewers`).

### Changed

- `.claude/commands/paper/loop.md` Step A item 2 extends the response-suite handoff from `/paper:triage` then `/paper:rebut` to also name `/paper:consistency` over the in-scope paper once the reviewer edits land (reviewer-driven edits leave stale summaries in untouched front matter, discussion, and conclusion) and `/paper:letter` to close the round, and notes that this is the same order `/paper:triage` itself recommends so the two routings agree.
- `VERSION`, `SKILL.md` `metadata.version`, and the `README.md` badge now report 1.38.0.

## [1.37.0] - 2026-07-21

Adds the precision budget as a named editing axis, drawn from author feedback on a pattern of LLM bad writing: the insistence on technical correctness everywhere. Uniform full precision suits a contract or a technical manual, not a research paper, where precision is purchased with the reader's attention and the opening is where attention is most expensive; early material may legitimately run at directional truth to keep the reader engaged, provided the precise version later reads as a sharpening, never a reversal. The release operationalizes that with a decidable gate (the refinement-vs-retraction test: would an expert who read the full paper call the intro compressed, or misled?), a distinction between scope hedges (deferrable with one forward pointer) and epistemic hedges (relocatable, never deletable), a per-zone precision gradient, a debt ledger tying every early simplification to its discharging full statement, and a mechanically censused hedge lexicon whose edit is classification, never find-and-delete. The rationale ships with the rules on purpose: without it, a hedge pass strips the calibration that keeps claims honest, which is the exact failure the existing qualifier-is-content rule exists to prevent. Everything stays inside the existing constraints: a deferred hedge is a logged deletion under constraint 6 and a relocation under the altitude gates, an unpaid debt is an `Author questions` item (drafting the missing full statement would be new substance), and the argumentation principle's calibration still binds in both directions.

### Added

- `references/precision-budget.md`, a new reference. It carries the attention-economics rationale (post-rigorous writing, the inverted pyramid), the precision gradient (abstract and introduction run at directional truth; methods and results at full precision; discussion and limitations are where hedges live), the scope-versus-epistemic hedge distinction, the refinement-vs-retraction test with its three outcomes (deferrable, keeper, or an author question when the claim itself outruns the evidence), the one-signpost rule (one short forward pointer, never an inline caveat clause, never two pointers), the precision debt ledger with its four dispositions, the first-mention-versus-full-statement rule composed with say-it-once, the hedge lexicon (may, might, could, suggest, potentially, arguably, generally, typically, in many cases, to some extent, it is important to note) with a mechanical Grep census followed by per-hit classification, the scope and stage gates inherited from altitude, and a worked pair that shows both branches (the clean version when the body preserves the claim, the boundary marker when it does not, the epistemic keeper when the evidence is only suggestive).

### Changed

- `SKILL.md` gains a "Precision budget: spend precision where attention is expensive" section after Altitude, framed as a third axis (the keep-test asks whether a unit survives, the altitude test asks where it belongs, the precision budget asks how much precision each location must carry), with the gradient, the refinement-vs-retraction gate, the two hedge kinds, the signpost ceiling, the debt ledger, the lexicon census, a bad-versus-good introduction pair, and the load trigger for `references/precision-budget.md`. The preflight checklist gains a precision-budget check, the restraint checklist gains a matching verbatim-return gate that also names the keepers a kept hedge may be, and the change-rationale reason set gains "a hedge deferred under the refinement test" and "kept calibration on a central claim" so both directions of the edit have sanctioned justifications.
- `references/altitude.md`'s compose paragraph grows from two neighbours to three (the refinement test decides whether a qualification may leave its claim; altitude picks the destination), and the send-each-caveat-home directive now points at the refinement test as the deferral decision rule.
- `references/subtraction.md`'s qualifier section gains the front-matter third option: a scope qualifier may defer to the section that establishes the claim, with one forward pointer, under the refinement test and the altitude gates, never as a cut, and an epistemic hedge on a central claim is not deferrable at all.
- `references/edit-checks.md` check 6 (progressive disclosure) names the refinement test as its decision rule for which caveats defer.
- `references/ai-tells-to-avoid.md`'s hedged-confidence tell and diagnostic checklist gain the zone-scoped lexicon with the classify-never-blanket-delete instruction, keeping the lexicon words distinct from the banned lists (they are wrong only at the wrong altitude).
- `references/consistency-checks.md` gains the unpaid-precision-debt check in both directions (an early directional claim never stated in full; an early claim whose full statement reverses sign, guts magnitude, or collapses scope).
- `references/cold-read.md`'s delight audit adds the early-spent hedge to the taxes and quotes each unpaid or retracted simplification, since a front-to-back read is where the whole-paper test is answerable.
- `.claude/agents/paper-reviser.md`'s reference-trigger list adds `precision-budget.md`.
- `VERSION`, `SKILL.md` `metadata.version`, and the `README.md` badge now report 1.37.0.

## [1.36.0] - 2026-07-21

Adds detection of the interrupted clause, drawn from author feedback on a Discussion section that still read awkward after several passes. The awkward sentences shared one shape: an interruptive appositive or parenthetical opening in a gap in the clause core, between an element and the word that completes it (a subject and its verb, a verb and its object, an object and its complement, and so on) ("Our data leave that question, calibration, open"; "Those conditions, team literacy, schema design, and task framing, are the axes most likely to vary"). They survived every pass because the skill had no check for within-clause parse difficulty, and worse, three mechanisms actively protected them: the voice-tic machinery filed the habit as authorial voice, the restraint checklist could not see it and returned the sentence verbatim, and the change-rationale schema offered no sanctioned reason for the fix, so "better, not merely different" reverted it. The 1.35.0 "buried thesis" pattern already covered the special case of a paragraph's main-claim sentence being the most nested; this release generalizes it to any interrupted clause and closes the three protection gaps. All of it stays inside the existing constraints: a change in syntax that carries every scope and calibration qualifier through unchanged, no new substance.
...
- `SKILL.md`'s "Writing quality" principle now names clause-core adjacency, every element beside the word that completes it (subject-verb, linking-verb-complement, verb-object or particle, object-complement), and points at the new pattern. The restraint checklist gains a matching verbatim-return gate (no interruptive appositive or parenthetical holds a clause's core open, separating any element from the word that completes it; each clause, embedded ones included, parses in one pass, though a brief parenthetical the reader resolves may stay), the voice-tic exclusion list now names interruptive appositives and parentheticals that split a clause's core so the habit is not preserved as voice, the change-rationale reason set gains "a reunited clause core" so the edit has a sanctioned justification, and the read-cold pass gains a per-sentence clause-core-split check on the rewrite that closes the gap by bringing the completing word up or moving the interrupter out.
