...
- `SKILL.md`'s "Writing quality" principle now names subject-verb, verb-object, and object-complement adjacency and points at the new pattern. The restraint checklist gains a matching verbatim-return gate (no interruptive appositive or parenthetical splits the core of a clause, a subject from its verb, a verb from its object or particle, or an object from its complement; each clause, embedded ones included, parses in one pass), the voice-tic exclusion list now names interruptive appositives and parentheticals that split a clause's core so the habit is not preserved as voice, the change-rationale reason set gains "a reunited clause core" so the edit has a sanctioned justification, and the read-cold pass gains a per-sentence clause-core-split check on the rewrite.
