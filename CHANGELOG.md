...
- `install.sh`: `ensure_skill_linked` tolerates a conflict on one target, so an unmanaged `~/.agents/skills/paper-revision-editor` from another tool no longer aborts command registration before the `~/.claude` link Claude Code needs is created.
