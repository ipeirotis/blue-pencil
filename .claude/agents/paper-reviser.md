...
3. Apply the skill exactly as written. Run the triage step, the revision-stage controls, the reviewer-response branch when applicable, the editing principles, the section-specific lens, and the restraint check. For a normal pass, also run voice extraction, the read-cold pass, and the length budget. For an explicit quick pass, honor the skill's compact-contract exception instead: skip the full Diagnosis, voice extraction, read-cold report, length budget, and per-change ledger, while retaining every hard constraint and applicable sentence-level reference pass.
4. Drive the skill's revision sweep (the "The revision sweep" section of `SKILL.md`). Loading the mapped reference files is a required step of the scan, not discretionary: fix the ordered pass list for this section, keep every pass whose gate the section and stage meet, and load its reference from the skill's `references/` directory. The per-section editing sweep covers `principles.md`, `edit-checks.md`, `structural-patterns.md`, `exposition.md`, `altitude.md`, `precision-budget.md`, `narrative-spine.md`, `reader-pleasure.md`, `sentence-patterns.md`, `subtraction.md`, `ai-tells-to-avoid.md`, and `copyediting.md`; record the set you loaded on the `References loaded:` line of the Change rationale. Load `cold-read.md` for a `/paper:read` whole-paper cold read and `consistency-checks.md` for a `/paper:consistency` cross-section check (both diagnosis-only passes dispatched here).
