...
- An **epistemic hedge** says "we are not sure this is true". On a peripheral claim it follows the ordinary rules. On the paper's central claim it can never be edited away: the only question is where it lives, and the refinement test below decides it. When the front-matter claim would overclaim without it (evidence that is only suggestive, or a borderline or fragile result whose uncertainty is part of the finding, per the fragility rule in `references/altitude.md`), it stays inline with the claim even at the top of the paper, as the epistemic keeper of the classification below; it relocates to where the evidence is presented only when the clean front-matter statement still passes the refinement test without it.
