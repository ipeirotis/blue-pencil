...
Distinguish an echo from a genuine next step. "Accuracy fell 18 points" followed by "so the speed gain did not translate into more correct answers" is an inference the first sentence did not state, and the keep-test keeps it (criterion 1). "Accuracy fell 18 points" followed by "the interface produced no detectable accuracy gain" is the same claim twice, and one of them goes.

## Universal qualifiers are filler, not scope

The qualifier-is-content rule protects scope and calibration qualifiers: "on the held-out set", "correlational", "in our sample" bound a claim to what the evidence bears, so removing one changes the claim and counts as a deletion. A universal qualifier does the opposite. It is true of every study, so it bounds nothing. "At this sample size" (every result is at its sample size), "observable" on effort or effect (every measured thing is observable), and "to some extent" add no scope the claim did not already carry. These fail the keep-test and compress away freely.

A dataset-scope qualifier is not in this class. "In our data" and "in our sample" bound the claim to the data observed even with no contrast stated nearby, so cutting one broadens the claim from the dataset to a general statement; keep it protected unless the author has marked it redundant. The test that separates the two: does the qualifier rule out a reading the reader would otherwise take? A scope qualifier does ("held-out, not training"; "the data we observed, not data in general"), so it is content; a universal qualifier does not, so it is filler. When in doubt, treat it as scope and keep it, since cutting a real scope qualifier is the costlier error (criterion 2).
