...
| Before | After |
|---|---|
| On this evidence the interface relocates the visible work of querying rather than demonstrably removing work: for our SQL-literate participants a verification burden survives the switch to natural language, and an NLIDB that hides the generated SQL would remove the very step these users relied on to trust the answer. | The interface moves the work of querying rather than removing it. Our SQL-literate users still had to verify the generated SQL, and one that hid it would remove the very step they relied on to trust the answer. |
