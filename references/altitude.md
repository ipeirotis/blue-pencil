...
**Keep one number per claim; send the rest down.** A confidence interval, a range, or a secondary p-value stacked onto a single headline figure is usually a Results move: state the point estimate and one marker, and let the precision ride down into the body. Two things keep the interval at altitude, so check them first: a venue whose abstract format requires it (some structured and clinical abstracts do, per `target_venue`), and a result whose point is the precision itself, where the interval width is the finding. Absent both, the interval goes down.
