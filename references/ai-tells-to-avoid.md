...
- Hedged confidence ("This may potentially suggest that..."). Pick a confidence level and own it. In an abstract, introduction, or contribution paragraph, run the zone-scoped hedge lexicon in `references/precision-budget.md` (may, might, could, potentially, arguably, generally, typically, in many cases, to some extent) and classify each hit as epistemic keeper, deferrable scope, or filler; these words are not banned, they are wrong only at the wrong altitude, so never blanket-delete them.
...
- [ ] In an abstract, introduction, or contribution paragraph: hedge-lexicon words (may, might, could, potentially, arguably, generally, typically, in many cases, to some extent)? Classify per `references/precision-budget.md` (keep, defer with one signpost, or cut as filler); never blanket-delete.
