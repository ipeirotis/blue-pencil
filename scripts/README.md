...
| `check-protected.sh` | Diff protected content between each example's input block and its `Revised text` block: citation keys (LaTeX including optional arguments, pandoc `@key` in any form, and plain author-year runs), cross-reference keys and prose callouts ("Table 4", "Appendix C"), math spans (dollar, `\(...\)`, `\[...\]`, and whole environments including contents), macros with their arguments (prose-argument macros like `\caption` by name only, honoring the caption carve-out), quoted text, and sign-, range-, and percent-aware numbers. The mechanical version of the skill's "No protected content changed" preflight, with an in-script exceptions list for legitimate flagged changes. | `make check-protected` |
