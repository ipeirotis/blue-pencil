...
| `check-examples.sh` | Lock `examples/` to the strict output format in `SKILL.md`: heading order, `Word count:` shape, question marks, banned tells in every edited paragraph of the `Revised text` block (paragraphs returned verbatim from the input are exempt), the stage-appropriate `Voice tics:` and `Reader map:` headers, extraction-line consistency, the `Added bridges:` line, and the `Reader map:` template. | `make check-examples` |
| `check-protected.sh` | Diff protected content between each example's input block and its `Revised text` block: citation keys (LaTeX, pandoc, and plain author-year runs), cross-reference keys and prose callouts ("Table 4", "Appendix C"), math spans (dollar, `\(...\)`, `\[...\]`, environment delimiters), and sign-, range-, and percent-aware numbers. The mechanical version of the skill's "No protected content changed" preflight, with an in-script exceptions list for legitimate flagged changes. | `make check-protected` |
